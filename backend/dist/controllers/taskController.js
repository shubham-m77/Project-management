...
const getAccessibleProject = async (projectId, userId) => {
    if (!(0, mongoose_1.isValidObjectId)(projectId))
        return null;
    return Project_1.default.findOne({
        _id: projectId,
        $or: [{ owner: userId }, { "members.user": userId }],
    });
};
const canManageTasks = (project, userId) => project.owner.equals(userId) || project.members.some((member) => member.user.equals(userId) && member.role === "admin");
